...
for i in {1..399}; do 
  echo "\"command\": \"clang++ -c `pwd`/gen_src/source${i}.cpp -o `pwd`/build/obj/Debug/source${i}.o -MF `pwd`/build/obj/Debug/source${i}.d -MMD -MP\"," >> compile_commands.json
  echo "\"directory\": \"`pwd`/build\"," >> compile_commands.json
  echo "\"file\": \"`pwd`/gen_src/source${i}.cpp\"" >> compile_commands.json
  echo "}, {" >> compile_commands.json
done

echo "\"command\": \"clang++ -c `pwd`/gen_src/source400.cpp -o `pwd`/build/obj/Debug/source400.o -MF `pwd`/build/obj/Debug/source400.d -MMD -MP\"," >> compile_commands.json
echo "\"directory\": \"`pwd`/build\"," >> compile_commands.json
echo "\"file\": \"`pwd`/gen_src/source400.cpp\"" >> compile_commands.json
echo "} ]" >> compile_commands.json
